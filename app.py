...
def init_db():
    with sqlite3.connect("wheelsup.db") as con:
        ...
        cur.execute('''CREATE TABLE IF NOT EXISTS comments (id INTEGER PRIMARY KEY, post_id INTEGER, user_id INTEGER, content TEXT, created_at TEXT)''')
        cur.execute('''CREATE TABLE IF NOT EXISTS likes (user_id INTEGER, post_id INTEGER, PRIMARY KEY(user_id, post_id))''')
        cur.execute('''CREATE TABLE IF NOT EXISTS follows (follower_id INTEGER, followee_id INTEGER, PRIMARY KEY(follower_id, followee_id))''')
        cur.execute('''CREATE TABLE IF NOT EXISTS trips (id INTEGER PRIMARY KEY, user_id INTEGER, title TEXT, description TEXT, trip_date TEXT, location TEXT)''')
...
def index():
    ...
    con = sqlite3.connect("wheelsup.db")
    cur = con.cursor()
    cur.execute("SELECT posts.id, users.name, posts.content, posts.image, posts.created_at, users.id FROM posts JOIN users ON posts.user_id = users.id ORDER BY posts.created_at DESC")
    posts = cur.fetchall()
    likes = {row[0]: row[1] for row in con.execute("SELECT post_id, COUNT(*) FROM likes GROUP BY post_id")}
    comments = {}
    for row in con.execute("SELECT post_id, users.name, content, created_at FROM comments JOIN users ON comments.user_id = users.id ORDER BY created_at"):
...
def profile(user_id):
    ...
    posts = cur.fetchall()
    return render_template_string(PROFILE_TEMPLATE, name=name, posts=posts)
...
def explore():
    ...
    users = cur.fetchall()
    cur.execute("SELECT posts.id, users.name, posts.content, posts.image, posts.created_at FROM posts JOIN users ON posts.user_id = users.id ORDER BY RANDOM() LIMIT 10")
    posts = cur.fetchall()
...
def dm(user_id):
    ...
    if request.method == "POST":
        ...
        cur.execute("INSERT INTO messages (sender_id, receiver_id, message, created_at) VALUES (?, ?, ?, ?)",
                    (user[0], user_id, msg, str(datetime.datetime.now())))
        con.commit()
    cur.execute("SELECT sender_id, message, created_at FROM messages WHERE (sender_id=? AND receiver_id=?) OR (sender_id=? AND receiver_id=?) ORDER BY created_at",
...
def inbox():
    ...
    con = sqlite3.connect("wheelsup.db")
    cur = con.cursor()
    cur.execute("SELECT DISTINCT receiver_id FROM messages WHERE sender_id=? UNION SELECT DISTINCT sender_id FROM messages WHERE receiver_id=?", (user[0], user[0]))
    users = cur.fetchall()
...
def notifications():
    ...
    con = sqlite3.connect("wheelsup.db")
    cur = con.cursor()
    cur.execute("SELECT users.name, 'liked your post', posts.created_at FROM likes JOIN posts ON likes.post_id = posts.id JOIN users ON likes.user_id = users.id WHERE posts.user_id=?", (user[0],))
    likes = cur.fetchall()
    cur.execute("SELECT users.name, 'commented on your post', comments.created_at FROM comments JOIN posts ON comments.post_id = posts.id JOIN users ON comments.user_id = users.id WHERE posts.user_id=?", (user[0],))
    comments = cur.fetchall()
    cur.execute("SELECT users.name, 'followed you', NULL FROM follows JOIN users ON follows.follower_id = users.id WHERE followee_id=?", (user[0],))
...
LOGIN_TEMPLATE = TAILWIND_HEAD + '''<html><body class="flex items-center justify-center h-screen bg-gray-100">
...
REG_TEMPLATE = TAILWIND_HEAD + '''<html><body class="flex items-center justify-center h-screen bg-gray-100">
<div class="bg-white p-6 rounded shadow w-full max-w-sm">
...
FEED_TEMPLATE = TAILWIND_HEAD + '''<html><body class="bg-gray-50">
...
CHAT_TEMPLATE = TAILWIND_HEAD + '''<html><body class="bg-gray-100 p-4">
...
INBOX_TEMPLATE = TAILWIND_HEAD + '''<html><body class="bg-gray-100 p-4">
...
EXPLORE_TEMPLATE = TAILWIND_HEAD + '''<html><body class="bg-gray-100 p-4">
...
TRIP_TEMPLATE = TAILWIND_HEAD + '''<html><body class="bg-gray-100 p-4">
...
NOTIFICATION_TEMPLATE = TAILWIND_HEAD + '''<html><body class="bg-gray-100 p-4">
<h2 class="text-2xl font-bold mb-4">Notifications</h2>
<div class="space-y-2">
{% for n in notes %}
  <div class="bg-white p-3 rounded shadow">
    <strong>{{ n[0] }}</strong> {{ n[1] }} <span class="text-xs text-gray-400">{{ n[2] or '' }}</span>
  </div>
{% endfor %}
</div>
</body></html>'''

PROFILE_TEMPLATE = TAILWIND_HEAD + '''<html><body class="bg-gray-100 p-4">
